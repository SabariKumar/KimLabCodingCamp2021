...
def atom_groups():
	C_group = ['C', 'Si', 'Ge', 'Sn', 'Pb']
	N_group = ['N', 'P', 'As', 'Sb', 'Bi']
	O_group = ['O', 'S', 'Se', 'Te', 'Po']
	F_group = ['F', 'Cl', 'Br', 'I', 'At']

	return C_group,N_group,O_group,F_group

# AUTOMATICALLY SETS THE CHARGE FOR METAL COMPLEXES
def rules_get_charge(mol,args,log):
	# charge valid ligand atoms (right now, the code doesn't account for B atoms linked to metals
	# but the code recognizes B- groups in the ligands)
				...
				if neighbour.GetTotalValence()== 4:
					if neighbour.GetSymbol() in C_group:
						...
						for inside_neighbour in neighbour.GetNeighbors():
							if inside_neighbour.GetSymbol() in N_group or inside_neighbour.GetSymbol() == 'B':
	...
	# recognizes charged charged atoms in metal ligands (added to the first metal of the list as default)
